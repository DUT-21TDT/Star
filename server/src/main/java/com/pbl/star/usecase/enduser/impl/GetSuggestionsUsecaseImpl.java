...
public class GetSuggestionsUsecaseImpl implements GetSuggestionsUsecase {
    ...
    public List<OnSuggestionProfileResponse> getFollowSuggestions(int limit) {
        ...
        List<OnSuggestionProfile> suggestions;
        try {
            suggestions = followService.suggestFollowCache(currentUserId, limit);
        } catch (RedisConnectionFailureException ex) {
            suggestions = followService.suggestFollow(currentUserId, limit);
        }
        return suggestions.stream().map(mapper::toDTO).toList();
    }
}
